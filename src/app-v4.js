...
function diagnostics(){const center=currentModelCenter(new THREE.Vector3()),jointNames=['upperarm_l','hand_l','middle_01_l','upperarm_r','hand_r','middle_01_r','calf_l','foot_l','ball_l','calf_r','foot_r','ball_r'],joints={};for(const name of jointNames)joints[name]=wp(name).toArray();const meshMinY=new THREE.Box3().setFromObject(model).min.y;return{ready,exercise:EXERCISES[current].id,supportMode,meshMinY,camera:{panEnabled:controls.enablePan,target:controls.target.toArray(),center:center.toArray(),distanceToCenter:controls.target.distanceTo(center),distance:camera.position.distanceTo(controls.target)},joints}}
